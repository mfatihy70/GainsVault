...
export function Stopwatch({
  ...
  useEffect(() => {
    ...
    if (isRunning) {
      intervalRef.current = setInterval(() => {
        ...
        if (endTimeMode == TIME_MODE_AUTOMATIC) setEndTime(Date.now())
      }, 1000)
    } else {
  ...
  const getStyle = (id) => ({
    ...
    backgroundColor:
      selection === id ? "rgba(255, 193, 7, 0.219)" : "transparent",
    cursor: "pointer",
  })
      ...
      <Modal show={showModal} onHide={handleClose}>
        ...
        <Modal.Body className="bg-dark text-light border-warning">
          ...
          <Row className="gap-2 mx-auto">
            <Col
              className="border border-warning rounded p-3 mb-3 d-flex justify-content-between align-items-center"
              onClick={() => setSelection(TIME_SELECTION_START)}
              style={getStyle(TIME_SELECTION_START)}
            >
              <div> Start </div>
          ...
          <Col className="mb-3 border border-warning rounded p-2">
            <h4>
              {selection === TIME_SELECTION_START ? "StartTime" : "EndTime"}
            </h4>
